...
// Assignment represents a policy assignment for a scheduled override as returned
// by the API. Note the assigned user is reported in the "user" field.
type Assignment struct {
	Policy   string `json:"policy,omitempty"`   // policy slug
	User     string `json:"user,omitempty"`     // assigned username
	Team     string `json:"team,omitempty"`     // assigned team
	Assigned bool   `json:"assigned,omitempty"` // assignment status
	SelfURL  string `json:"_selfUrl,omitempty"` // hypermedia link
}

// UpdateAssignmentPayload is the request body for updating an override
// assignment. Per the API, only the username is required; AcceptOverlap
// acknowledges assigning someone who already has an absence scheduled.
...
func (c *Client) CreateScheduledOverride(ctx context.Context, override *ScheduledOverridePayload) (*ScheduledOverride, *RequestDetails, error) {
	...
	// (the spec says "schedule"); fall back for spec compatibility.
...
func (c *Client) GetScheduledOverride(ctx context.Context, publicID string) (*ScheduledOverride, *RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "GET", "v1/overrides/"+url.PathEscape(publicID), bytes.NewBufferString("{}"), nil)
	if err != nil {
		return nil, details, err
	}
...
func (c *Client) DeleteScheduledOverride(ctx context.Context, publicID string) (*RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "DELETE", "v1/overrides/"+url.PathEscape(publicID), bytes.NewBufferString("{}"), nil)
	return details, err
}
...
func (c *Client) GetScheduledOverrideAssignments(ctx context.Context, publicID string) ([]Assignment, *RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "GET", "v1/overrides/"+url.PathEscape(publicID)+"/assignments", bytes.NewBufferString("{}"), nil)
	if err != nil {
		return nil, details, err
	}
...
func (c *Client) GetScheduledOverrideAssignment(ctx context.Context, publicID string, policySlug string) (*Assignment, *RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "GET", "v1/overrides/"+url.PathEscape(publicID)+"/assignments/"+url.PathEscape(policySlug), bytes.NewBufferString("{}"), nil)
	if err != nil {
		return nil, details, err
	}
...
// UpdateScheduledOverrideAssignment updates an assignment for a scheduled
// override. The request body carries the username to assign (and optionally
// AcceptOverlap); the policy is identified by policySlug in the URL path.
func (c *Client) UpdateScheduledOverrideAssignment(ctx context.Context, publicID string, policySlug string, payload *UpdateAssignmentPayload) (*Assignment, *RequestDetails, error) {
	jsonAssignment, err := json.Marshal(payload)
	if err != nil {
		return nil, nil, err
	}

	details, err := c.makePublicAPICall(ctx, "PUT", "v1/overrides/"+url.PathEscape(publicID)+"/assignments/"+url.PathEscape(policySlug), bytes.NewBuffer(jsonAssignment), nil)
	if err != nil {
		return nil, details, err
	}
...
func (c *Client) DeleteScheduledOverrideAssignment(ctx context.Context, publicID string, policySlug string) (*Assignment, *RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "DELETE", "v1/overrides/"+url.PathEscape(publicID)+"/assignments/"+url.PathEscape(policySlug), bytes.NewBufferString("{}"), nil)
	if err != nil {
		return nil, details, err
	}
