...
// GetAlertRule gets an alert rule by ID. The API has no GET-by-ID endpoint, so
// this lists all rules and filters by ID.
...
func (c *Client) UpdateAlertRule(ctx context.Context, ruleID string, rule *AlertRulePayload) (*AlertRule, *RequestDetails, error) {
	...
	details, err := c.makePublicAPICall(ctx, "PUT", "v1/alertRules/"+url.PathEscape(ruleID), bytes.NewBuffer(jsonRule), nil)
	if err != nil {
		return nil, details, err
	}
...
func (c *Client) DeleteAlertRule(ctx context.Context, ruleID string) (*DeleteAlertRuleResponse, *RequestDetails, error) {
	details, err := c.makePublicAPICall(ctx, "DELETE", "v1/alertRules/"+url.PathEscape(ruleID), bytes.NewBufferString("{}"), nil)
	if err != nil {
		return nil, details, err
	}
